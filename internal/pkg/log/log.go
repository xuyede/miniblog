...
// 设置一个空白标识符_，类型为Logger
// 赋值时会检查右侧的值是否满足左侧的类型约束
// 如果 *zapLogger 没有实现 Logger 接口中的所有方法（Debugw、Infow、Warnw 等），编译就会报错。
...
// 使用 zap.Config的Build 方法创建 *zap.Logger 对象，并将其封装在 zapLogger 结构体中返回
