...
VERSION_PACKAGE=github.com/xuyede/miniblog/pkg/version
...
ca: ## 生成 CA 文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/ca.key -out $(OUTPUT_DIR)/cert/ca.csr \
		-subj "//C=CN/ST=Guangdong/L=Shenzhen/O=devops/OU=it/CN=127.0.0.1/emailAddress=will3virgo@163.com" # 2. 生成请求文件
	...
	@openssl req -new -key $(OUTPUT_DIR)/cert/server.key -out $(OUTPUT_DIR)/cert/server.csr \
		-subj "//C=CN/ST=Guangdong/L=Shenzhen/O=serverdevops/OU=serverit/CN=127.0.0.1/emailAddress=will3virgo@163.com" # 6. 生成服务端向 CA 申请签名的 CSR
